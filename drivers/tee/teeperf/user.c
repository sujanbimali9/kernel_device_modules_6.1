	...
	u32 high_freq)
{
	...
	if (group == CPU_SUPER_GROUP) {
		freq_level_index = g_teeperf_cpu_index.cpu_super_index;
		...
		freq_level_index = g_teeperf_cpu_index.cpu_big_index;
		if (map == CPU_4_3_1_MAP) {
		...
		freq_level_index = g_teeperf_cpu_index.cpu_little_index;
		if (map == CPU_4_3_1_MAP) {
