...
static irqreturn_t mt6379_irq_threaded_handler(int irq, void *d)
{
	...
	for (i = 0; i < ARRAY_SIZE(mt6379_irq_ind_rg_maps); i++) {
		...
		ret = regmap_raw_read(regmap, map->evt_rg_base, evts + evt_offs,
				      map->rg_num);
		if (ret) {
			dev_info(dev, "Failed to read evts %d (%d)\n", i, ret);
			return IRQ_NONE;
		}
		...
		if (ret) {
			dev_info(dev, "Failed to write evts %d (%d)\n", i, ret);
			return IRQ_NONE;
		}
	}
	...
	if (ret)
		dev_info(dev, "Failed to set IRQ retrigger (%d)\n", ret);
...
static void mt6379_irq_bus_sync_unlock(struct irq_data *d)
{
	...
	if (ret)
		dev_info(dev, "Failed to config for hwirq %ld\n", d->hwirq);
...
static int mt6379_init_irq_chip(struct mt6379_data *data)
{
	...
	if (!data->irq_domain)
		return rt_err_probe(dev, -ENOMEM, "Failed to create IRQ domain\n");

	ret = devm_add_action_or_reset(dev, mt6379_free_irq_chip, data);
	if (ret)
		return rt_err_probe(dev, ret, "Failed to add devm for irq chip release\n");
...
int mt6379_device_init(struct mt6379_data *data)
{
	...
	if (ret)
		return rt_err_probe(dev, ret, "Failed to read vendor info\n");

	if ((vid & MT6379_VENID_MASK) != MT6379_VENDOR_ID)
		return rt_err_probe(dev, -ENODEV, "VID not matched 0x%02x\n", vid);

	dev_set_drvdata(dev, data);

	ret = mt6379_init_irq_chip(data);
	if (ret)
		return rt_err_probe(dev, ret, "Failed to init irq chip\n");
	...
	if (ret)
		return rt_err_probe(dev, ret, "Failed to create test_mode attribute\n");
