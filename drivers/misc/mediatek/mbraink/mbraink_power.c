...
int mbraink_power_spm_info_init(void)
{
	struct lpm_sys_res_mbrain_dbg_ops *sys_res_mbrain_ops = NULL;
	...
	if (g_data_size != 0 && g_data_size <= SPM_TOTAL_SZ) {
		pr_notice("[Mbraink][SPM] alloc g_data_size(%d)\n", g_data_size);
...
int mbraink_power_spm_info_deinit(void)
{
	if (g_spm_raw != NULL) {
		vfree(g_spm_raw);
		g_spm_raw = NULL;
		...
		if (g_data_size <= SPM_TOTAL_SZ) {
			...
			if (sys_res_mbrain_ops &&
			   ...
			   sys_res_mbrain_ops->get_data(g_spm_raw, g_data_size) != 0) {
				pr_info("[Mbraink][SPM] get_data fail\n");
				goto End;
			}
		}
	}
