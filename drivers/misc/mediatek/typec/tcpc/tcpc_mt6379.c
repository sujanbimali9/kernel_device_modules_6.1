...
static inline u32 rg_xlate(struct mt6379_tcpc_data *ddata, u32 reg)
{
	...
	dev_info(ddata->dev, "%s: failed to translate reg0x%04x\n", __func__, reg);
	return reg;
}
...
static int mt6379_tcpc_init_irq(struct mt6379_tcpc_data *ddata)
{
	...
	if (ret < 0) {
		dev_info(ddata->dev, "failed to get irq pd_evt\n");
		return ret;
	}
	ddata->irq = ret;
	...
	if (ret < 0) {
		dev_info(ddata->dev, "failed to request irq %d\n", ddata->irq);
...
static int mt6379_check_revision(struct mt6379_tcpc_data *ddata)
{
	...
	if (ret < 0) {
		dev_info(ddata->dev, "failed to read vid(%d)\n", ret);
		return ret;
	}
	if (id != MT6379_VID) {
		dev_info(ddata->dev, "incorrect vid(0x%04X)\n", id);
		return -ENODEV;
	}

	ret = mt6379_read16(ddata, TCPC_V10_REG_PID, &id);
	if (ret < 0) {
		dev_info(ddata->dev, "failed to read pid(%d)\n", ret);
		return ret;
	}
	if (id != MT6379_PID) {
		dev_info(ddata->dev, "incorrect pid(0x%04X)\n", id);
		return -ENODEV;
	}

	ret = mt6379_read16(ddata, TCPC_V10_REG_DID, &id);
	if (ret < 0) {
		dev_info(ddata->dev, "failed to read did(%d)\n", ret);
		return ret;
	}
...
static int mt6379_tcpc_probe(struct platform_device *pdev)
{
	...
	if (!ddata->rmap) {
		dev_info(ddata->dev, "failed to get regmap\n");
		return -ENODEV;
	}

	ret = mt6379_check_revision(ddata);
	if (ret < 0) {
		dev_info(ddata->dev, "failed to check revision(%d)\n", ret);
		return ret;
	}
	...
	if (ret < 0) {
		dev_info(ddata->dev, "failed to parse dt(%d)\n", ret);
		return ret;
	}
	...
	if (IS_ERR(ddata->adc_iio)) {
		...
		dev_info(ddata->dev, "failed to get adc iio(%d)\n", ret);
		return ret;
	}

	ret = mt6379_register_tcpcdev(ddata);
	if (ret < 0) {
		dev_info(ddata->dev, "failed to register tcpcdev(%d)\n", ret);
		return ret;
	}

	ret = mt6379_sw_reset(ddata);
	if (ret < 0) {
		dev_info(ddata->dev, "failed to reset sw(%d)\n", ret);
		goto err;
	}
	...
	if (ret < 0) {
		dev_info(ddata->dev, "failed to init irq\n");
		goto err;
	}
