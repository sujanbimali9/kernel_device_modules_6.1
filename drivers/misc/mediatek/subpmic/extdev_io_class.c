...
static int extdev_io_read(struct extdev_io_device *extdev, char *buf)
{
	...
	cnt += snprintf(buf + cnt, IO_BUFFER_SIZE - cnt, "0x");

	for (i = 0; i < extdev->size; i++)
		cnt += scnprintf(buf + cnt, IO_BUFFER_SIZE - cnt, "%02x,", *(data + i));

	cnt += scnprintf(buf + cnt, IO_BUFFER_SIZE - cnt, "\n");
	return ret;
}
			      ...
			      char *buf)
{
	...
	case EXTDEV_IO_DESC_REG:
		cnt += scnprintf(buf + cnt, IO_BUFFER_SIZE - cnt, "0x%04x\n", extdev->reg);
		break;
	case EXTDEV_IO_DESC_SIZE:
		cnt += scnprintf(buf + cnt, IO_BUFFER_SIZE - cnt, "%d\n", extdev->size);
		break;
	case EXTDEV_IO_DESC_DATA:
		ret = extdev_io_read(extdev, buf);
		break;
	case EXTDEV_IO_DESC_TYPE:
		cnt += snprintf(buf + cnt, IO_BUFFER_SIZE - cnt, "%s\n", extdev->desc->typestr);
		break;
	case EXTDEV_IO_DESC_LOCK:
		cnt += snprintf(buf + cnt, IO_BUFFER_SIZE - cnt, "%d\n", extdev->access_lock);
		break;
	default:
		ret = -EINVAL;
		break;
...
{
	struct extdev_desc extdev_desc = { .io_read = NULL, .io_write = NULL };
