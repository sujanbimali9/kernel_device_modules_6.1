...
FPSGO_COM_SYSFS_WRITE_POLICY_CMD(mfrc_active_by_pid, 5, 0, 1);
		...
		char *buf)
{
	...
	for (rbn = rb_first(rbr); rbn; rbn = rb_next(rbn)) {
		...
		length = scnprintf(temp + pos,
			...
			"%dth\ttgid:%d\tbypass_non_SF:%d\tcontrol_api_mask:%d\tcontrol_hwui:%d\tapp_cam_align:(%d,%d)\tapp_min_fps:%d\tmfrc%d\tts:%llu\n",
			i,
			iter->tgid,
