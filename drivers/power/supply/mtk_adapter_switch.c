...
int mtk_selected_adapter_ready(struct mtk_charger *info)
{
	...
	case RUN_ON_PD:
		...
		if (info->adapter_dev[PD] &&
			ta_type == MTK_PD_APDO) {
			adapter_switch_err("%s: run pd pps adapter\n", __func__);
			...
			ta_type != MTK_PD_APDO) {
			adapter_switch_err("%s: run pd adapter\n", __func__);
			info->select_adapter = info->adapter_dev[PD];
			info->select_adapter_idx = PD;
			info->ta_capability = WO_APDO_TA;
			ret = 1;
		} else {
			adapter_switch_err("%s: note : can't find pd adapter\n",
				__func__);
			info->ta_capability = STD_TA;
			ret = -1;
		}
