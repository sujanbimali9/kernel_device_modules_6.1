...
static int mt6379_ufcs_config_baud_rate(struct ufcs_dev *ufcs, enum ufcs_baud_rate rate)
{
	...
	case UFCS_BAUD_RATE_38400:
		...
		break;
	default:
		dev_info(data->dev, "Not supported rate %d\n", rate);
		return -EINVAL;
	}
...
static int mt6379_ufcs_send_hard_reset(struct ufcs_dev *ufcs, enum ufcs_hard_reset_type type)
{
	...
	case UFCS_HARD_RESET_TO_CABLE:
		...
		break;
	default:
		dev_info(data->dev, "Unknown hardreset type %d\n", type);
		return -EINVAL;
	}
...
static irqreturn_t mt6379_ufcs_evt_handler(int irq, void *data)
{
	...
	if (events & MT6379_EVT_UFCS_DATA_READY) {
		...
		if (!ret)
			...
			dev_info(udata->dev, "Failed to get rx message\n");
...
out:
	...
	if (ret)
		dev_info(udata->dev, "Failed to unmask UFCS IRQ\n");

	/* Retrigger */
	ret = regmap_write(regmap, MT6379_REG_SPMI_TXDRV2, MT6379_RCS_INT_DONE_MASK);
	if (ret)
		dev_info(udata->dev, "Failed to do IRQ retrigger\n");
...
static int mt6379_ufcs_probe(struct platform_device *pdev)
{
	...
	if (!data->regmap)
		return rt_err_probe(dev, -ENODEV, "Failed to init regmap\n");

	irq = platform_get_irq(pdev, 0);
	if (irq < 0)
		return rt_err_probe(dev, irq, "Failed to get ufcs irq\n");
	...
	if (IS_ERR(data->port))
		return rt_err_probe(dev, PTR_ERR(data->port),  "Failed to register port\n");

	ret = devm_request_threaded_irq(data->dev, irq, NULL, mt6379_ufcs_evt_handler,
				   IRQF_ONESHOT, dev_name(data->dev), data);
	if (ret)
		return rt_err_probe(dev, ret, "Failed to request irq\n");
