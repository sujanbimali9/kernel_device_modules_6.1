...
static void gauge_irq_sync_unlock(struct irq_data *data)
{
	...
	if (ret)
		dev_info(priv->dev, "Failed to set/clr BAT%d irq con %d\n",
...
static int mt6379_gauge_dump_registers(struct mt6379_priv *priv)
{
	...
	for (i = 0; i < ARRAY_SIZE(regs); i++) {
		...
		if (ret) {
			dev_info(priv->dev, "%s, Failed to read BAT%d RG:%s(0x%X)\n",
...
static irqreturn_t gauge_irq_thread(int irq, void *data)
{
	...
	if (ret) {
		dev_info(priv->dev, "%s, Failed to read BAT%d INT status\n",
			 __func__, bat_idx + 1);
		return IRQ_HANDLED;
	}
	...
	for (i = 0; i < NUM_IRQ_REG; i++) {
		ret = regmap_write(priv->regmap,
				   ...
				   mask[i]);
		if (ret)
			dev_info(priv->dev, "%s, Failed to mask BAT%d irq[%d]\n",
	...
	for (i = 0; i < NUM_IRQ_REG; i++) {
		...
		for (j = 0; j < 8; j++) {
			...
			dev_info(priv->dev, "%s, BAT%d handle gauge irq(reg:0x%X, bit:%d)\n",
				 __func__, bat_idx + 1,
	...
	for (i = 0; i < NUM_IRQ_REG; i++) {
		ret = regmap_write(priv->regmap,
				   ...
				   mask[i]);
		if (ret)
			dev_info(priv->dev, "%s, Failed to unmask BAT%d irq[%d]\n",
				 __func__, bat_idx + 1, i);
	}
	...
	ret = regmap_raw_write(priv->regmap, rg[bat_idx][MT6379_REG_BM_TOP_INT_STATUS0],
			       status_buf, sizeof(status_buf));
	if (ret)
		dev_info(priv->dev, "%s, Failed to clear BAT%d INT status\n", __func__, bat_idx + 1);
...
static int gauge_add_irq_chip(struct mt6379_priv *priv)
{
	...
	for (i = 0; i < NUM_IRQ_REG; i++) {
		...
		if (ret) {
			dev_info(priv->dev, "%s, Failed to disable BAT%d irq con [%d]\n",
				 __func__, bat_idx + 1, i);
			return ret;
		}
		...
		if (ret) {
			dev_info(priv->dev, "%s, Failed to init BAT%d irq mask [%d]\n",
				 __func__, bat_idx + 1, i);
			return ret;
		}
	}
	...
	if (!priv->domain) {
		dev_info(priv->dev, "%s, Failed to create BAT%d IRQ domain\n",
			 __func__, bat_idx + 1);
		return -ENOMEM;
	}
	...
	if (ret) {
		dev_info(priv->dev, "%s, Failed to request BAT%d IRQ %d for %s: %d\n",
...
static int pre_gauge_update(struct mtk_gauge *gauge)
{
	...
	if (i == max_retry_cnt) {
		dev_info(dev, "%s, BAT%d timeout! last BM[0x6F]=0x%x\n",
			 __func__, bat_idx + 1, rdata);
...
static int reg_to_current(struct mtk_gauge *gauge, unsigned int regval)
{
	...
	bm_trace("[%s]regval:0x%x,uvalue16:0x%x,dvalue:0x%x,temp_value:0x%x,retval:0x%x,is_charging:%d\n",
...
static u8 get_rtc_spare0_fg_value(struct mtk_gauge *gauge)
{
	...
	if (IS_ERR(cell)) {
		bm_err("%s, Failed to get rtc cell\n", __func__);
		return 0;
	}
	...
	nvmem_cell_put(cell);
	if (IS_ERR(buf)) {
		bm_err("%s, Failed to read rtc cell\n", __func__);
		return 0;
	}
...
static void set_rtc_spare0_fg_value(struct mtk_gauge *gauge, u8 val)
{
	...
	nvmem_cell_put(cell);
	if (ret != length)
		bm_err("%s, Failed to write rtc cell\n", __func__);
}
...
static int fgauge_set_info(struct mtk_gauge *gauge, enum gauge_property ginfo, unsigned int value)
{
	...
	bm_debug("%s, mt6379-gauge BAT%d set info:%d v:%d\n", __func__, bat_idx + 1, ginfo, value);
...
static int fgauge_get_info(struct mtk_gauge *gauge, enum gauge_property ginfo, int *value)
{
	...
	bm_debug("%s, mt6379-gauge BAT%d get info:%d v:%d\n",
		 __func__, bat_idx + 1, ginfo, *value);
	return 0;
}
...
static unsigned int instant_current_for_car_tune(struct mtk_gauge *gauge)
{
	...
	if (ret)
		dev_info(priv->dev, "%s, Failed to read BAT%d FGADC_CUR_CON0 (ret:%d)\n",
			 __func__, bat_idx + 1, ret);

	post_gauge_update(gauge);

	bm_err("%s, BAT%d reg_value=0x%04x\n", __func__, bat_idx + 1, reg_value);
...
static int calculate_car_tune(struct mtk_gauge *gauge)
{
	...
	if (gauge->hw_status.meta_current != 0) {
		...
		if (dvalue != 0) {
			...
			bm_err("[fgauge_meta_cali_car_tune_value][%d] meta:%d, adc:%lld, UNI_FGCUR:%d, r_fg_value:%d\n",
...
static int instant_current(struct mtk_gauge *gauge, int *val, enum fg_cic_idx cic_idx)
{
	...
	bm_debug("%s, mt6379-gauge BAT%d, r_fg=%d, car_tune:%d, CIC%d current:%d\n",
...
static int read_hw_ocv_6379_plug_in(struct mtk_gauge *gauge)
{
	...
	ret = regmap_raw_read(gauge->regmap, rg[bat_idx][MT6379_REG_AUXADC_ADC_OUT_BAT_PLUGIN_PCHR],
			      &regval, sizeof(regval));
	if (ret) {
		dev_info(priv->dev, "Failed to read BAT%d plugin PCHR\n", bat_idx + 1);
		return ret;
	}
	...
	bm_err("[oam] %s (BAT%d pchr): adc_result_reg=%d, adc_result=%d, start_sel=%d, rdy=%d\n",
...
static int read_hw_ocv_6379_power_on(struct mtk_gauge *gauge)
{
	...
	if (is_ship_rst) {
		bm_err("%s: BAT%d before cali, is_ship_rst:%d, offset_trim:0x%x, gain_err:0x%x, adc_result_reg:0x%x\n",
		...
		adc_result_reg = adc_result_reg * (ADC_PRECISE + priv->gain_err) / ADC_PRECISE +
				 offset_trim;
		bm_err("%s: BAT%d after cali, adc_result_reg:0x%x\n",
...
static int read_hw_ocv_6379_power_on_rdy(struct mtk_gauge *gauge)
{
	...
	bm_err("[%s] BAT%d pwron_PCHR_rdy %d\n", __func__, bat_idx + 1, pon_rdy);
					       ...
					       int _thr_mv)
{
	...
	if (gauge->thr_reg >= 32768) {
		bm_err("[%s] BAT%d nag_c_dltv_thr mv=%d ,thr_reg=%d, limit thr_reg to 32767\n",
	...
	bm_debug("[fg_bat_nafg][%s] BAT%d time[%d] zcv[%d:%d] thr[%d:%d] 26_0[0x%x]\n",
					 ...
					 int fg_zcv_car_th)
{
	...
	bm_debug("[FG_ZCV_INT][%s] BAT%d det_time %d mv %d reg %lld 30_00 0x%x\n",
...
static int fgauge_get_time(struct mtk_gauge *gauge, unsigned int *ptime)
{
	...
	bm_debug("[%s] BAT%d regval:0x%x rtime:0x%llx 0x%x!\r\n",
...
static int nafg_check_corner(struct mtk_gauge *gauge)
{
	...
	bm_debug("%s, BAT%d nag_vbat:%d nag_zcv:%d get_c_dltv_mv:%d setto_cdltv_thr_mv:%d, RG[0x%x]\n",
...
static int coulomb_get(struct mtk_gauge *gauge, struct mtk_gauge_sysfs_field_info *attr, int *val)
{
	...
	if (ret) {
		dev_info(priv->dev, "%s, Failed to read BAT%d FGADC CAR (ret:%d)\n",
			 __func__, bat_idx + 1, ret);
		return ret;
	}
			       ...
			       int *data)
{
	...
	if (ret) {
		dev_info(dev, "%s, Failed to read BAT%d FGADC_IAVG_CON1(ret:%d)\n",
			 __func__, bat_idx + 1, ret);
		return ret;
	}
	...
	if (iavg_vld) {
		...
		bm_debug("[fg_get_current_iavg] BAT%d fg_iavg_ma:%lld, fg_iavg_reg:%lld, fg_iavg_reg_tmp:%lld\n",
		...
		bm_debug("[fg_get_current_iavg] BAT%d fg_iavg_ma %lld\n",
			 bat_idx + 1, fg_iavg_ma);

		if (sign_bit)
			fg_iavg_ma = 0 - fg_iavg_ma;

		bm_debug("[fg_get_current_iavg] BAT%d fg_iavg_ma:%lld, fg_iavg_reg:%lld, r_fg_value:%d, 27_16:0x%x, 15_00:0x%x\n",
		...
		bm_debug("[fg_get_current_iavg] BAT%d PMIC_FG_IAVG_VLD == 1\n", bat_idx + 1);
	} else {
		ret = instant_current(gauge, &gauge->fg_hw_info.current_1, FG_CIC1);
		if (ret) {
			dev_info(dev, "%s, Failed to get BAT%d CIC1(ret:%d)\n",
				 __func__, bat_idx + 1, ret);
			return ret;
		}
		...
		bm_debug("[fg_get_current_iavg] BAT%d PMIC_FG_IAVG_VLD != 1, avg %d, current_1 %d\n",
	...
	*data = gauge->fg_hw_info.current_avg;
	...
	bm_debug("[fg_get_current_iavg] BAT%d current_avg:%d valid:%d is_bat_charging:%d\n",
...
static signed int fg_set_iavg_intr(struct mtk_gauge *gauge_dev, void *data)
{
	...
	if (ret) {
		dev_info(priv->dev, "%s, Failed to get BAT%d IAVG(ret:%d)\n",
			__func__, bat_idx + 1, ret);
		return ret;
	}
...
/* / fg_cust_data.car_tune_value / priv->unit_fg_iavg  = fg_iavg_reg  */
	...
	bm_debug("[FG_IAVG_INT][%s] BAT%d iavg %d iavg_gap %d iavg_ht %lld iavg_lt %lld fg_iavg_reg_ht %lld fg_iavg_reg_lt %lld\n",
		 __func__, bat_idx + 1, iavg, iavg_gap, iavg_ht, iavg_lt,
		 fg_iavg_reg_ht, fg_iavg_reg_lt);

	bm_debug("[FG_IAVG_INT][%s] BAT%dlt_28_16 0x%x lt_15_00 0x%x ht_28_16 0x%x ht_15_00 0x%x\n",
...
static int hw_info_set(struct mtk_gauge *gauge_dev, struct mtk_gauge_sysfs_field_info *attr, int en)
{
	...
	if (ret) {
		dev_info(dev, "%s, Failed to get BAT%d CIC2(ret:%d)\n",
			 __func__, bat_idx + 1, ret);
		return ret;
	}
	...
	if (ret) {
		dev_info(dev, "%s, Failed to get BAT%d IAVG(ret:%d)\n",
			 __func__, bat_idx + 1, ret);
		return ret;
	}
	...
	if ((is_iavg_valid == 1) && (gauge_status->iavg_intr_flag == 0)) {
		...
		bm_debug("[read_fg_hw_info] BAT%d set first fg_set_iavg_intr:%d, %d\n",
		...
		disable_gauge_irq(gauge_dev, FG_IAVG_H_IRQ);
		disable_gauge_irq(gauge_dev, FG_IAVG_L_IRQ);
		bm_debug("[read_fg_hw_info] BAT%d double check first fg_set_iavg_intr:%d, %d\n",
			 bat_idx + 1, is_iavg_valid, gauge_status->iavg_intr_flag);
	}

	bm_debug("[read_fg_hw_info] BAT%d third check first fg_set_iavg_intr:%d, %d\n",
		 bat_idx + 1, is_iavg_valid, gauge_status->iavg_intr_flag);
	...
	bm_debug("[FGADC_intr_end][read_fg_hw_info] BAT%d curr_1:%d curr_2:%d Iavg:%d sign:%d car:%d ncar:%d time:%d\n",
					...
					struct mtk_gauge_sysfs_field_info *attr, int threshold)
{
	...
	bm_err("BAT%d car:%d carR:%lld r:%lld\n", bat_idx + 1, threshold, car, carReg);
			   ...
			   int en)
{
	...
	bm_debug("%s, BAT%d zcv_intr_en_cnt:%d en:%d\n", __func__, bat_idx + 1, *cnt, en);
	...
	*cnt = en ? *cnt + 1 : *cnt - 1;
	...
	bm_debug("[FG_ZCV_INT][fg_set_zcv_intr_en] BAT%d en:%d\n", bat_idx + 1, en);
				  ...
				  int zcv_avg_current)
{
	...
	bm_debug("[%s] BAT%d current:%d, fg_zcv_det_time:%d, fg_zcv_car_th:%d\n",
			    ...
			    int val)
{
	...
	bm_err("[fgauge_read_RTC_boot_status]%s, BAT%d spare0 0x%x 0x%x, spare3 0x%x 0x%x\n",
	...
	if (((after_rst_spare3_reg != (spare3_reg | 0x80)) || (after_rst_spare0_reg != temp_value))
		...
		bm_err("[%s][retry] BAT%d spare0 0x%x 0x%x, spare3 0x%x 0x%x\n",
			   ...
			   int *nafg_c_dltv)
{
	...
	*nafg_c_dltv = nag_c_dltv_mv_value;
	...
	bm_debug("[fg_bat_nafg][%s] BAT%d mV:Reg[%d:%d] [b10:%d][26_16(0x%04x) 15_00(0x%04x)]\n",
			 ...
			 int *nag_dltv)
{
	...
	*nag_dltv = reg_to_mv_value(gauge, reg_value);
	...
	bm_debug("[fg_bat_nafg][%s] BAT%d mV:Reg [%d:%d] [%d:%d]\n",
			...
			int *nag_cnt)
{
	...
	*nag_cnt = NAG_C_DLTV_CNT & AUXADC_NAG_CNT_MASK;
	bm_debug("[fg_bat_nafg][%s] BAT%d %d [25_0 %d]\n",
			   ...
			   int *zcv_current)
{
	...
	if (gauge->gm->fg_cust_data.r_fg_value != priv->default_r_fg) {
		bm_debug("[fgauge_read_current] BAT%d Auto adjust value due to the Rfg is %d\n Ori curr=%d",
			 bat_idx + 1, gauge->gm->fg_cust_data.r_fg_value, dvalue);

		dvalue = (dvalue * priv->default_r_fg) / gauge->gm->fg_cust_data.r_fg_value;

		bm_debug("[fgauge_read_current] BAT%d new current=%d\n", bat_idx + 1, dvalue);
	}

	bm_debug("[fgauge_read_current] BAT%d ori current=%d\n", bat_idx + 1, dvalue);
	dvalue = ((dvalue * gauge->gm->fg_cust_data.car_tune_value) / 1000);
	bm_debug("[fgauge_read_current] BAT%d final current=%d (ratio=%d)\n",
...
static int zcv_get(struct mtk_gauge *gauge_dev, struct mtk_gauge_sysfs_field_info *attr, int *zcv)
{
	...
	bm_err("[oam] %s BAT%d BATSNS ZCV (pchr):adc_result_reg=%d, adc_result=%d\n",
			...
			int *val)
{
	...
	if (gm != NULL)
		if (_hw_ocv < 28000 && (gm->disableGM30 == 0)) {
			bm_err("[%s] BAT%d ERROR, _hw_ocv=%d  src:%d, force use swocv\n",
	...
	*val = _hw_ocv;
	...
	bm_err("[%s] BAT%d g_fg_is_charger_exist:%d _hw_ocv_chgin_rdy:%d pl:%d %d\n",
	       __func__, bat_idx + 1, fg_is_charger_exist, _hw_ocv_chgin_rdy,
	       zcvinfo->pl_charging_status, zcvinfo->moniter_plchg_bit);
	bm_err("[%s] BAT%d _hw_ocv:%d _sw_ocv:%d now_thr:%d\n",
	       __func__, bat_idx + 1, _prev_hw_ocv, _sw_ocv, now_thr);
	bm_err("[%s] BAT%d _hw_ocv:%d _hw_ocv_src:%d _prev_hw_ocv:%d  _prev_hw_ocv_src:%d _flag_unreliable:%d\n",
	       __func__, bat_idx + 1, _hw_ocv, _hw_ocv_src, _prev_hw_ocv,
	       _prev_hw_ocv_src, _flag_unreliable);
	bm_err("[%s] BAT%d _hw_ocv_75_pon_rdy:%d _hw_ocv_75_pon:%d _hw_ocv_75_plugin:%d _hw_ocv_chgin:%d _sw_ocv:%d now_temp:%d now_thr:%d\n",
	       __func__, bat_idx + 1, _hw_ocv_75_pon_rdy, _hw_ocv_75_pon,
		_hw_ocv_75_plugin, _hw_ocv_chgin, _sw_ocv, now_temp, now_thr);
	bm_err("[%s] BAT%d _hw_ocv_75_before_chgin %d\n",
			   ...
			   int *val)
{
	...
	if (IS_ERR(gauge->chan_ptim_r)) {
		bm_err("%s, BAT%d auxadc chan: imix_r(ptim_r) error\n",
		       __func__, bat_idx + 1);
		return -EOPNOTSUPP;
	}
	...
	if (ret)
		bm_err("%s, Failed to read BAT%d auxadc imix_r(ptim_r), ret=%d\n",
		       __func__, bat_idx + 1, ret);

	bm_err("%s, BAT%d imix_r(ptim_r) val:%d, ret=%d\n", __func__, bat_idx + 1, *val, ret);
	return ret;
}
				    ...
				    struct mtk_gauge_sysfs_field_info *attr, int *val)
{
	...
	if (IS_ERR(gauge->chan_ptim_bat_voltage)) {
		bm_err("%s, BAT%d auxadc chan: imp(ptim_bat_volt) error\n",
		       __func__, bat_idx + 1);
		return -EOPNOTSUPP;
	}
	...
	if (ret)
		bm_err("%s, Failed to read BAT%d auxadc imp(ptim_bat_volt), ret=%d\n",
		       __func__, bat_idx + 1, ret);

	bm_err("%s, BAT%d imp(ptim_bat_volt) val:%d, ret=%d\n",
	       __func__, bat_idx + 1, *val, ret);
	return ret;
}
			  ...
			  int *val)
{
	...
	*val = rtc_ui_soc;
	...
	if (rtc_ui_soc > 100 || rtc_ui_soc < 0)
		bm_err("%s, BAT%d ERR! rtc=0x%x, ui_soc=%d\n",
		       __func__, bat_idx + 1, rtc_value, rtc_ui_soc);
	else
		bm_debug("%s, BAT%d rtc=0x%x, ui_soc=%d\n",
			 __func__, bat_idx + 1, rtc_value, rtc_ui_soc);
		       ...
		       int threshold)
{
	...
	bm_debug("[fg_set_vbat2_l_th] BAT%d thr:%d [0x%x %d 0x%x %d 0x%x]\n",
		       ...
		       int threshold)
{
	...
	bm_debug("[fg_set_vbat2_h_th] BAT%d thr:%d [0x%x %d 0x%x %d 0x%x]\n",
...
static int en_l_vbat_set(struct mtk_gauge *gauge, struct mtk_gauge_sysfs_field_info *attr, int val)
{
	...
	bm_debug("%s, BAT%d en_l_vbat_cnt:%d en:%d\n", __func__, bat_idx + 1, cnt, val);
	cnt = val ? cnt + 1 : cnt - 1;
					       ...
					       int *val)
{
	...
	while (abs(cnt) < max_cnt) {
		...
		if (ret) {
			dev_info(priv->dev, "%s, Failed to get BAT%d cic1 (ret:%d)\n",
				 __func__, bat_idx + 1, ret);
			return ret;
		}

		bm_err("%s, BAT%d cic1 = %d\n", __func__, bat_idx + 1, value);
		if (abs(value) > 500) {
			bm_err("%s, BAT%d cic1 out of range(%d)\n",
			       __func__, bat_idx + 1, value);
			return -EINVAL;
		}
		...
		if (ret < 0) {
			bm_err("%s, Failed to get BAT%d vbat_mon\n",
			       __func__, bat_idx + 1);
			return ret;
		}
		...
		ret = iio_read_channel_processed(gauge->chan_bat_voltage,
						 &auxadc_vbat);
		if (ret < 0) {
			bm_err("%s, Failed to get BAT%d auxadc_vbat(%d)\n",
			       __func__, bat_idx + 1, ret);
			return ret;
		}
		...
		if (chg_vbat_max - chg_vbat_min > HTOL_THRESHOLD_MAX ||
			...
			bm_err("%s, BAT%d vbat_diff min/max out of range\n",
			       __func__, bat_idx + 1);
			return ret;
		}
		...
		if (abs(vbat_diff) > HTOL_THRESHOLD_MAX || abs(vbat_diff) < HTOL_THRESHOLD_MIN) {
			bm_err("%s, BAT%d vbat_diff is out of range(%d), no need to calibrate\n",
	...
	if (abs(gain_err_diff) > HTOL_CALI_MAX) {
		bm_err("%s, BAT%d gain_err_diff out of theshold(%d), adjust HTOL_THRESHOLD_MAX\n",
...
static int mt6379_auxadc_init_vbat_calibration(struct mt6379_priv *priv)
{
	...
	if (offset_trim >= 0x4000) {
		bm_err("%s, BAT%d  before handle offset trim signed, offset_trim:0x%x\n",
			   ...
			   int *val)
{
	...
	if (IS_ERR(gauge->chan_bif)) {
		bm_err("%s, BAT%d auxadc chan: vref(bif) error\n",
		       __func__, bat_idx + 1);
		return -EOPNOTSUPP;
	}
	...
	if (ret < 0)
		bm_err("%s, Failed to read BAT%d auxadc vref(bif), ret=%d\n",
		       __func__, bat_idx + 1, ret);

	bm_debug("%s, BAT%d vref(bif) val:%d, ret=%d\n",
		 __func__, bat_idx + 1, *val, ret);
	return ret;
}
				       ...
				       struct mtk_gauge_sysfs_field_info *attr, int *val)
{
	...
	if (IS_ERR(gauge->chan_bat_temp)) {
		bm_err("%s, BAT%d auxadc chan: baton(bat_temp) error\n",
		       __func__, bat_idx + 1);
		return -EOPNOTSUPP;
	}
	...
	bm_debug("%s, BAT%d baton(bat_temp) val:%d, ret=%d\n",
		 __func__, bat_idx + 1, *val, ret);
	return ret;
}
...
static int bat_vol_get(struct mtk_gauge *gauge, struct mtk_gauge_sysfs_field_info *attr, int *val)
{
	...
	if (IS_ERR(gauge->chan_bat_voltage)) {
		bm_debug("%s, BAT%d auxadc chan: batsns(bat_volt) error\n",
		       __func__, bat_idx + 1);
		return -EOPNOTSUPP;
	}
	...
	if (ret < 0) {
		bm_err("%s, Failed to read BAT%d auxadc batsns(bat_volt), ret=%d\n",
		       __func__, bat_idx + 1, ret);
		return ret;
	}
	...
	if (*val < 1000) {
		if (t1 == 0) {
			...
			bm_err("[%s] BAT%d vbat_mon = %d(%d)\n",
			       __func__, bat_idx + 1, vbat_mon, ret);
			for (i = 0; i < ARRAY_SIZE(dump_reg); i++) {
				ret = regmap_read(gauge->regmap, dump_reg[i] + bank_offset, &data);
				bm_err("[%s] BAT%d addr:0x%4x, data:0x%x(%d)\n",
	...
	bm_debug("%s, BAT%d batsns(bat_volt) val:%d, ret=%d\n",
		 __func__, bat_idx + 1, *val, ret);
	return ret;
}
				    ...
				    struct mtk_gauge_sysfs_field_info *attr, int val)
{
	...
	bm_debug("[%s] BAT%d FG_CAR = 0x%x:%d uvalue32_car_msb:0x%x 0x%x 0x%x\r\n",
	...
	bm_debug("[%s] BAT%d low=0x%x:%d diff_car=0x%llx:%lld\r\n",
	...
	bm_debug("[%s] BAT%d final low=0x%x:%d car=0x%llx:%lld\r\n",
		 __func__, bat_idx + 1, lowbound, lowbound, car, car);

	bm_debug("[%s] BAT%d final low 0x%x 0x%x 0x%x car=0x%llx\n",
	...
	bm_debug("[%s] BAT%d low:0x%x 0x%x car_value:%d car:%d irq:%d\r\n",
				    ...
				    struct mtk_gauge_sysfs_field_info *attr, int val)
{
	...
	bm_debug("[%s] BAT%d upper = 0x%x:%d diff_car=0x%llx:%lld\r\n",
	...
	bm_debug("[%s] BAT%d final upper = 0x%x:%d car=0x%llx:%lld\r\n",
		 __func__, bat_idx + 1, upperbound, upperbound, car, car);

	bm_debug("[%s] BAT%d final upper 0x%x 0x%x 0x%x car=0x%llx\n",
	...
	bm_debug("[%s] BAT%d high:0x%x 0x%x car_value:%d car:%d irq:%d\r\n",
			    ...
			    int *val)
{
	...
	if (ret) {
		dev_info(priv->dev, "%s, Failed to get BAT%d CIC2, ret = %d\n",
			 __func__, bat_idx + 1, ret);
		return ret;
	}
...
{
	struct mt6379_priv *priv = container_of(gauge, struct mt6379_priv, gauge);
	int is_charger_exist = 0, bat_flag = 0;
	const unsigned int bat_idx = priv->desc->bat_idx;
	u16 rev_val = 0;
	...
	bm_err("BAT%d bat_plug:%d chr:%d info0:0x%x\n",
...
static long compat_adc_cali_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	bm_notice("%s, BAT%d 32bit IOCTL, cmd=0x%08x (%s)\n",
		  __func__, bat_idx + 1, cmd, get_cmd_name(cmd));
	if (!file->f_op || !file->f_op->unlocked_ioctl) {
		bm_err("%s, BAT%d file has no f_op or no f_op->unlocked_ioctl.\n",
		       __func__, bat_idx + 1);
		return -ENOTTY;
	}
	...
	case Set_BAT_DISABLE_NAFG:
	case Set_CARTUNE_TO_KERNEL:
		bm_notice("%s, BAT%d send to unlocked_ioctl cmd=0x%08x (%s)\n",
			  __func__, bat_idx + 1, cmd, get_cmd_name(cmd));
		return file->f_op->unlocked_ioctl(file, cmd, (unsigned long)compat_ptr(arg));
	default:
		bm_err("%s, BAT%d unknown IOCTL: 0x%08x (%s), %d\n",
...
static long adc_cali_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
	case Get_META_BAT_VOL:
		adc_out_data[0] = gauge_get_int_property(GAUGE_PROP_BATTERY_VOLTAGE);
		...
		bm_notice("%s, **** unlocked_ioctl: BAT%d Get_META_BAT_VOL Done!\n",
			  __func__, bat_idx + 1);
		break;
	case Get_META_BAT_SOC:
		...
		bm_notice("%s, **** unlocked_ioctl: BAT%d Get_META_BAT_SOC Done!\n",
			  __func__, bat_idx + 1);
		break;

	case Get_META_BAT_CAR_TUNE_VALUE:
		adc_out_data[0] = gm->fg_cust_data.car_tune_value;
		bm_err("%s, BAT%d Get_BAT_CAR_TUNE_VALUE, res=%d\n",
		...
		bm_notice("%s, **** unlocked_ioctl: BAT%d Get_META_BAT_CAR_TUNE_VALUE Done!\n",
			  __func__, bat_idx + 1);
		break;
	case Set_META_BAT_CAR_TUNE_VALUE:
		...
		if (temp_car_tune >= 900 && temp_car_tune <= 1100)
			...
			bm_err("%s, BAT%d car_tune_value invalid:%d\n",
		...
		bm_err("%s, **** unlocked_ioctl: BAT%d Set_BAT_CAR_TUNE_VALUE[%d], tmp_car_tune=%d result=%d, ret=%d\n",
	...
	case Set_BAT_DISABLE_NAFG:
		...
		bm_debug("%s, unlocked_ioctl: BAT%d Set_BAT_DISABLE_NAFG,isdisNAFG=%d [%d]\n",
	...
	case Set_CARTUNE_TO_KERNEL:
		...
		bm_err("%s, **** unlocked_ioctl: BAT%d Set_CARTUNE_TO_KERNEL[%d,%d], ret=%d\n",
		       __func__, bat_idx + 1, adc_in_data[0], adc_in_data[1], ret);
		break;
	default:
		bm_err("%s, **** unlocked_ioctl: BAT%d unknown IOCTL: 0x%08x\n",
...
static int adc_cali_cdev_init(struct mtk_battery *gm, struct platform_device *pdev)
{
	...
	if (ret)
		bm_err("%s, Error: BAT%d Can't Get Major number for adc_cali\n",
		       __func__, bat_idx + 1);
	...
	if (ret)
		bm_err("%s, BAT%d adc_cali Error: cdev_add\n",
		       __func__, bat_idx + 1);

	priv->fg_info.bat_cali_major = MAJOR(priv->fg_info.bat_cali_devno);
	priv->fg_info.bat_cali_class = class_create(THIS_MODULE, priv->desc->cdev_gauge_name);
...
static int bat_create_netlink(struct platform_device *pdev)
{
	...
	gauge->gm->mtk_battery_sk = netlink_kernel_create(&init_net, NETLINK_FGD, &cfg);
...
static int get_ptim_current(struct mtk_gauge *gauge)
{
	...
	dvalue = dvalue * -1;
	bm_debug("[%s] BAT%d ptim current:%d\n", __func__, bat_idx + 1, dvalue);
	...
	case POWER_SUPPLY_PROP_ENERGY_EMPTY_DESIGN:
		if (gm && val->intval != 0) {
			...
			if (gm->imix > 5500) {
				gm->imix = 5500;
				dev_info(priv->dev,
					 "%s, BAT%d imix reach limitation 5500, val:%d\n",
